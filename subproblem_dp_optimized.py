    ...
    def solveModelOpt(self, timeLimit):
        ...
        if NUMBA_AVAILABLE:
            ...
            if use_bidir and n_days >= 8:  # Bidir only useful for T >= 8
                ...
                # Prepare NL params
                is_nl = 1 if getattr(self, 'nl_spec', None) is not None else 0
                gamma_R = self.nl_spec['gamma_R'] if is_nl else 1.0
                gamma_C = self.nl_spec['gamma_C'] if is_nl else 1.0
                alpha_R = self.nl_spec['alpha_R'] if is_nl else 1.0
                e_max = self.nl_spec.get('e_max', 1.0) if is_nl else 1.0
                delta_flat = np.zeros(16, dtype=np.float64)
                if is_nl and 'delta' in self.nl_spec:
                    delta_mat = self.nl_spec['delta']
                    for s in range(4):
                        for s_prime in range(4):
                            if s < delta_mat.shape[0] and s_prime < delta_mat.shape[1]:
                                delta_flat[s * 4 + s_prime] = float(delta_mat[s, s_prime])
                ...
                fwd_states, fwd_costs, fwd_paths, n_fwd = forward_pass_numba(
                    ...
                    is_nl, gamma_R, gamma_C, alpha_R, delta_flat, e_max
                )
