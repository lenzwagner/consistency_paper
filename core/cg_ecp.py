...
def column_generation_ecp(data, demand_dict, eps, Min_WD_i, Max_WD_i, time_cg_init, max_itr, output_len, chi, threshold, time_cg, I, T, K, epsi, scale, k=2, sp_solver='mip', use_null_column=False, worker_groups=None):
    ...
    while modelImprovable and itr < max_itr:
        ...
        # Build SP (eps = 0.0 during optimization since ECP assumes 100% performance)
        subproblem = create_subproblem(sp_solver, duals_i, duals_ts, data, 1, itr, eps, Min_WD_i, Max_WD_i, chi)
        subproblem.buildModel()
        subproblem.addECPConstraint(k) # <-- Enforce ECP rolling window shift changes cap!

        # Save time to solve SP
        sub_start_time = time.time()
        if sp_solver.lower() in ['dp', 'labeling', 'labeling_bidir']:
            print("*{:^{output_len}}*".format(f"Warning: ECP constraint not fully supported in DP. Solving with {sp_solver}...", output_len=output_len))
    ...
    undercoverage_, understaffing_, perfloss_, consistency_, consistency_norm_, undercoverage_norm_, understaffing_norm_, perfloss_norm_, ls_perf, cumulative_total_ = master.calc_naive(ls_perf_, ls_sc, ls_rec, epsi, scale, worker_groups=worker_groups)
