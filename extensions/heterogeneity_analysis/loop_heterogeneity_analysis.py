...
def save_summary_csv(all_results):
    ...
    filename = os.path.join(results_dir, f'paper_text_metrics_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.csv')
    df_summary.to_csv(filename, index=False)
    print(f"\nSummary CSV for paper text saved to: {filename}")


def save_detailed_stats_csv(all_results, output_dir=None):
    ...
    for scenario_key, results in all_results.items():
        ...
        filename = os.path.join(output_dir, f'detailed_stats_{scenario_key}_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.csv')
...
if __name__ == "__main__":
    ...
    if RUN_ANALYSIS in ['baseline', 'all']:
        ...
        for scenario_key in ['homogen', 'heterogen_3cluster']:
            ...
            results_dir = os.path.join(script_dir, 'results')
            os.makedirs(results_dir, exist_ok=True)
            df.to_excel(os.path.join(results_dir, f'analysis_{scenario_key}_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.xlsx'), index=False)
    ...
    if RUN_ANALYSIS in ['sensitivity', 'all']:
        ...
        for scenario_key in ['none', 'low', 'moderate', 'high']:
            ...
            results_dir = os.path.join(script_dir, 'results')
            os.makedirs(results_dir, exist_ok=True)
            df.to_excel(os.path.join(results_dir, f'sensitivity_{scenario_key}_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.xlsx'), index=False)
    ...
    results_dir = os.path.join(script_dir, 'results')
    os.makedirs(results_dir, exist_ok=True)
    df_all.to_excel(os.path.join(results_dir, f'analysis_ALL_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.xlsx'), index=False)
