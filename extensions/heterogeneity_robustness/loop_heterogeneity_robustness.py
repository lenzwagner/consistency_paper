...
if __name__ == "__main__":
    ...
    for scenario_key, config in ROBUSTNESS_SCENARIOS.items():
        ...
        if res:
            ...
            out_path = os.path.join(results_dir, f'analysis_{scenario_key}_{datetime.now().strftime("%d_%m_%Y_%H-%M")}.xlsx')
            df_safe.to_excel(out_path, index=False)
    ...
    save_detailed_stats_csv(all_results, output_dir=results_dir)
    
    print(f"\nAll Robustness Execution complete! Time: {time.time() - start_time:.2f} seconds")
    print(f"Detailed stats files and complete trace results have been written to '{results_dir}'.")
